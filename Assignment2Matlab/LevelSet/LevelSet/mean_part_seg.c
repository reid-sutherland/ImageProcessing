...
float *Seg_Part_Mean(List_LevelSet *p_lls, Image *p_image, int p, float *F)
{
  ...
  for(k=0;k<p_lls->Num;k++){
	...
	if(l==k)
	  continue;
	vv = p_lls->l_levelset[l].p_param[2];
	F[k] = (vv-uu)*(I-(uu+vv)/2.0);
	break;
      }
    }
...
/*        Update parametres              */
/*****************************************/
int Update_Part_Mean(List_LevelSet *p_lls, Image *p_image,
		   int p){
  ...
  while((!flag_finish)&&(k<p_lls->Num)){
	...
	while((!flag_finish)&&(l<p_lls->Num)){
	  if(p_lls->l_levelset[l].U[p]>0.0){
	    p_lls->l_levelset[l].p_param[0] -= 1.0;
	    p_lls->l_levelset[l].p_param[1] -= p_image->I0[p];
